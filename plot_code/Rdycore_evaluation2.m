...
for g = 1:length(plot_gage_ids)
    ...
    ax = axes('Position',[pos_x pos_y ts_dx*0.91 ts_h*0.91]);
    hold on;
    ...
    RMSE_all(g) = RMSE;
    % Plot lines
    h1= plot(t, sim, '-', 'Color', colors{3}, 'LineWidth', 2);
    h2 = plot(t, obs, '-', 'Color', colors{4}, 'LineWidth', 2);
    ...
    if g == 1
        text(t(1), yl(2) + 0.1*range(yl), ...
            "(c)", ...
            'FontSize', 18, 'FontWeight', 'bold');
    end

    if g == 20
        legendp = legend([h1 h2],{'Rdycore', 'Obs'}, 'FontSize', 15, 'Location', 'west');
        legendp.Position(2) = legendp.Position(2) +0.026;

    end
    text(t(10), yl(2) - 0.1*range(yl), ...
    ...
    text(t(1250), yl(2) - 0.1*range(yl), ...
        ['#', num2str(g)], ...
        'FontSize', 15, 'FontWeight', 'bold', 'HorizontalAlignment', 'right', 'Color', [0.4 0.4 0.4]);
    % Formatting
    set(gca,'FontSize',9);
    if row < ts_rows-1
        set(gca,'XTickLabel',[]);
    ...
    if(col == 0)
    ylabel('Water height (m)','fontsize',15);
    end
    box on;
...
% Correlation map
ax1 = axes('Position',[map_x map_y_top map_w map_h]);
plot_global_map_site_ref2(lons(plot_gage_ids_1), lats(plot_gage_ids_1),lons(plot_gage_ids_2), lats(plot_gage_ids_2), R_all([1:15 19]),R_all([16:18 20]), 0, 1, 'R', 1, 1, '(a)');
title('Pearson Correlation (R)', 'FontSize', 14, 'FontWeight', 'bold');
nColors = 10;
...
% Colorbar for R
cb1 = colorbar('Position',[map_x+map_w+colorbar_gap map_y_top  colorbar_w map_h]);
%ylabel(cb1, 'R value', 'FontSize', 11);
cb1.Position(1) = cb1.Position(1)-0.04;  % example position, adjust to your layout
set(gca,'LineWidth',1)

% KGE map
ax2 = axes('Position',[map_x map_y_bot map_w map_h]);
plot_global_map_site_ref2(lons(plot_gage_ids_1), lats(plot_gage_ids_1),lons(plot_gage_ids_2), lats(plot_gage_ids_2), KGE_all([1:15 19]),KGE_all([16:18 20]), 0, 1, 'KGE', 1, 1, '(b)');
title('Modified Kling-Gupta Efficiency (KGE'')', 'FontSize', 14, 'FontWeight', 'bold');
colormap(ax2, cmap);

% Colorbar for KGE
cb2 = colorbar('Position',[map_x+map_w+colorbar_gap map_y_bot colorbar_w map_h]);
%ylabel(cb2, 'KGE value', 'FontSize', 11);
cb2.Position(1) = cb2.Position(1)-0.04;  % example position, adjust to your layout
set(gca,'LineWidth',1)
set(gca, 'FontSize', 15);
...
exportgraphics(gcf, 'Figure_1_gage_height_comparison.tiff', 'Resolution', 300);
hold off;
close all;
