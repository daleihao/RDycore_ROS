...
load("gages_xyz.mat");
load("gage_height_Rdycore_200m_v2.mat");
...
plot_global_map_site(lons(plot_gage_ids), lats(plot_gage_ids), R_all, 0, 1, 'R', 1, 1, '(a)');
title('Correlation (R)', 'FontSize', 14, 'FontWeight', 'bold');
nColors = 10;
...
cb1.Position(1) = cb1.Position(1)-0.03;  % example position, adjust to your layout
set(gca,'LineWidth',1)
% KGE map
ax2 = axes('Position',[map_x map_y_bot map_w map_h]);
plot_global_map_site(lons(plot_gage_ids), lats(plot_gage_ids), KGE_all, 0, 1, 'KGE', 1, 1, '(b)');
title('Kling-Gupta Efficiency (KGE)', 'FontSize', 14, 'FontWeight', 'bold');
colormap(ax2, cmap);

% Colorbar for KGE
cb2 = colorbar('Position',[map_x+map_w+colorbar_gap map_y_bot colorbar_w map_h]);
%ylabel(cb2, 'KGE value', 'FontSize', 11);
cb2.Position(1) = cb2.Position(1)-0.03;  % example position, adjust to your layout
set(gca,'LineWidth',1)
set(gca, 'FontSize', 15);
...
exportgraphics(gcf, 'gage_height_comparison_200.tiff', 'Resolution', 300);
hold off;
close all;

%% Helper function for metrics
function [R, KGE] = calc_KGE(sim, obs)
sim = sim(:);
obs = obs(:);
